...
- 期限は日付に加えて時刻も任意で設定可能（時刻未設定なら終日扱い）。期限を変更するとリマインド通知が再度そのタイミングで届くようリセットされる。通知タイミングはタスクごとに個別上書き可能（未指定ならチームの既定値）。
- カレンダーにも表示される短い「概要」欄。
...
- 月間カレンダーを表示エリア全幅で大きく表示し、日曜=赤・土曜=青・今日はアクセント色でハイライト。カレンダーはプロジェクトごとではなくチームに1つだけ存在し、チーム内の全プロジェクトのタスク期限とチームの予定を横断表示する。
- 期限つきタスクもカレンダー上に別色のチップとして自動表示（プロジェクト名+タスク名+概要、優先度で色分け、完了済みは打ち消し線）。
- 日付をクリックすると、その場所の近くにポップアップでその日の予定・期限タスクの一覧が表示され、その場で予定の追加・編集・削除が可能（タスクの編集は各プロジェクトのボードから）。
- 予定はプロジェクトに紐付かずチーム直下に作成され、開始・終了それぞれ時刻を任意設定でき、両方空欄なら終日として扱われる。詳細メモも記録可能。
- 前月/翌月への移動、「今日」ボタンでの即時ジャンプ。

### チーム・プロジェクト・メンバー管理

- 最上位の単位は「チーム」（Discordサーバー1つに1チームを想定）。チームの中に複数の「プロジェクト」（ボード単位）を作成でき、タスクはすべていずれかのプロジェクトに属する。
- チームに参加すると、そのチーム内の全プロジェクトへ自動的にアクセス可能になる（プロジェクトごとの個別招待・個別メンバーシップはない）。
- Discordユーザーネームを指定してメンバーを招待（7日間有効な招待、ロール指定可）。
- メンバーのロール変更・削除、チームからの脱退。
- メンバー一覧にはDiscordアバターと役割バッジを表示。

### チーム設定・プロジェクト管理

- チーム名の変更。
- 通知用Discord Webhook URLの登録（チーム内の全プロジェクトの通知が1つのチャンネルにまとまる）。
...
- **自動リマインド**: Supabaseのpg_cronが最大30分おきにチェックし、チームごとに設定したタイミング（期限/開始の何分〜何日前）になった、チーム内の全プロジェクトのタスク・チームの予定を、登録したWebhook宛のDiscordチャンネルへそれぞれ一度だけ自動投稿（Botプロセス不要、`pg_net`でHTTP POST）。期限やリマインド設定を変更すると再度通知対象になる。
- **スラッシュコマンド**: 連携済みのDiscordサーバーから `/task add title:<タイトル> due:<期限> project:<プロジェクト名（省略可）>` で、ログイン済み・チーム参加済みのユーザーが直接タスクを追加可能。
...
Supabase
  ├─ Postgres + RLS（supabase/migrations/001〜008、直近の008でチーム/プロジェクト構造へ再編）
  ├─ Edge Functions
...
| --- | --- |
| `owner` | チーム作成者。全操作可能 |
| `admin` | 招待、メンバー管理、編集、チーム設定・プロジェクト管理の変更が可能 |
| `member` | タスク・予定・リンク・添付の作成/更新/削除など通常編集が可能 |
| `viewer` | 読み取り専用（`is_team_editor()`/`is_group_editor()`を書き込み系RLSポリシーで使うことで強制） |

ロール・メンバーシップはチーム単位で、チーム参加と同時にそのチーム内の全プロジェクトへアクセスできます。`profiles`テーブルは自分自身、または同じチームに所属する相手のみ閲覧できます。
...
- チーム設定にDiscord Webhook URLを登録し、期限を今日にしたタスクを作成 → 最大30分待つ（またはSQL Editorで`select public.notify_due_tasks();`を手動実行）→ 通知が届くか確認。
- Discordの任意チャンネルで `/task add title:テスト` を実行 → 該当チームのプロジェクトにタスクが追加されるか確認。
...
- チーム設定（名前変更、Discord Webhook URL、連携サーバーID、タスク/予定それぞれのリマインドタイミング）、プロジェクトの作成/名前変更/削除
