...
- タスクカードは優先度（低・中・高・緊急、色分けされた左ボーダー）、期限、担当者アバター、コメント/リンク/添付件数を表示。
- 期限が今日を過ぎている/2日以内に迫っているタスクは、それぞれ強調表示。

### タスク詳細

- ステータス・優先度・期限・担当者をその場で変更（即保存）。
- 本文（詳細・メモ・仕様）は自由記述のテキストエリアで、入力停止から**3秒後に自動保存**。
- 同じタスクを複数人が同時に開くと「〇〇が入力中/閲覧中」とプレゼンス表示。
- `task_pages.version` を使ったバージョン競合検知。他人が先に保存していた場合は「最新版を見る」か「自分の内容で上書き」を選択可能。
- 関連リンク（`https://`のみ）、添付ファイル（Supabase Storage、上限10MB、実行ファイル系拡張子は拒否、署名付きURLでダウンロード）、コメントをタスクごとに管理。

### 予定（カレンダー）

- 月間カレンダーを大きく表示し、日曜=赤・土曜=青・今日はアクセント色でハイライト。
- 各日のマスに予定を時刻付きで表示（3件を超える分は「+N件」表示）。
- 日付をクリックすると右側パネルにその日の予定一覧が表示され、その場で追加・編集・削除が可能。
...
- グループ名の変更。
- 通知用Discord Webhook URLの登録（期限が近い/超過したタスクと開始間近の予定を自動通知）。
- 連携するDiscordサーバーID（Guild ID）の登録（`/task add`スラッシュコマンドの有効化）。

### Discordとの連携

- **自動リマインド**: Supabaseのpg_cronが最大30分おきにチェックし、期限が近い/超過したタスクと開始間近の予定を、登録したWebhook宛のDiscordチャンネルへ自動投稿（Botプロセス不要、`pg_net`でHTTP POST）。
...
- タスク・予定・コメント・リンク・添付メタ情報はPostgres Changes（Realtime）で反映（`useRealtimeInvalidate`がReact Queryのキャッシュを無効化）。
...
| `member` | タスク・予定・コメント・リンク・添付の作成/更新/削除など通常編集が可能 |
...
- `supabase/migrations/001_init.sql`と`002_redesign.sql`が適用されます。
